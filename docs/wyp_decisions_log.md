...
## 2026-09-03 — Install Housekeeping row extended: Apple/Safari guidance, platform-sensed Homepage/Desktop naming

Jim: "The app, when first used provides an ability to install a desktop/homepage icon for WYP for Android and for Windows... Could that process be extended to incorporate an explanation for Apple/Safari users when appropriate (can you sense for platform type?). Maybe there is a permanent Task for Desktop/Homepage Icon Installation (can you sense for it already being there to avoid repeating the install and creating a second icon?" Followed mid-turn by: "if you can identify the os from a web app, the actual Icon Installation could be named Homepage Icon Installation or Desktop Icon Installation."

Chromium's `beforeinstallprompt` (PWAProvider.tsx's `canInstall`/`promptInstall`, unchanged) has no equivalent on Apple platforms — Safari never fires it, on iOS/iPadOS or macOS — so there was never anything to sense a completed install from there either; the existing row simply never appeared. New `app/src/lib/platform.ts` (no migration, client-only heuristics): `isIOSDevice()` (classic iPhone/iPod UA, plus the iPadOS 13+ "MacIntel" UA quirk disambiguated via `maxTouchPoints`), `isMacOSDevice()` (the inverse — `MacIntel` with ≤1 touch point), `isSafariBrowser()` (UA regex excluding Chrome/Firefox/Edge's own Apple-platform UA tokens, since all three still contain the literal "Safari" substring there), `isAndroidDevice()`, `isMobileDevice()` (iOS or Android or a generic "Mobi" UA token), and `isStandaloneDisplay()` (`display-mode: standalone` media query OR iOS Safari's legacy `navigator.standalone` — the already-installed check, checked first and wins outright over every other branch, so a returning visitor is never offered a second icon on any platform, matching the existing Chromium behavior where `canInstall` also permanently goes false after a real install).

`MainScreen.tsx` gained `installGuidance` (`'none' | 'ios-safari' | 'ios-other' | 'mac-safari'`) and `installLabel` (`'Homepage' | 'Desktop'`, defaults `'Desktop'` — matches every existing account until the mount effect resolves, avoiding a flash of placeholder text), computed once in a mount effect deferred a microtask (same pattern `CreateRequestForm.tsx`'s own `voiceSupported` effect already established, to satisfy `react-hooks/set-state-in-effect` with no real async event to key off). The existing `canInstall` row and new `!canInstall && installGuidance !== 'none'` row are mutually exclusive by construction (Apple platforms never set `canInstall`, and everywhere `canInstall` can be true has no `installGuidance` branch that sets it) — both now read the same `{installLabel} Icon Installation` title and a label-matched hknote ("home screen" vs. "desktop"). The Apple-guidance row opens a new `.scrim`/`.modal` (§6.12) instructions dialog instead of calling `promptInstall()` (which doesn't exist on Safari) — three platform-specific bodies: `ios-safari` (Share icon → "Add to Home Screen" → "Add"), `ios-other` (explains only Safari can install on iOS, directs the visitor to open the site there), `mac-safari` (File menu → "Add to Dock…" → "Add"). Numbered steps use plain bold-prefixed `<p>` tags rather than `<ol>`/`<li>` — investigated first: only `.promo-features` has ever used a `<ul>` in this app, and it explicitly resets `list-style: none` and supplies its own bullet, leaving no confirmed precedent that native list markers render at all under this app's Tailwind v4 preflight; plain paragraphs sidestep an unverified rendering dependency rather than risk invisible numbers. `npx tsc --noEmit`/`npm run lint` clean. No mockup — the Install row has never had one, same as every other Housekeeping row's own live-only history.
...
**Search Text.** New `searchText` state, matching each row's own Description, case-insensitive substring — the same convention Main Screen's own search already uses. Sits beside Recipient/Requestor in one `.frow` (both share `.frow .ffloat`'s existing equal-width flex rule, 2026-08-11) when the type has a name field (Sent/Received); alone in the row for ToDos, which has none. Round-trip-only persistence (`ARCHIVE_SEARCH_KEY`, `isArchiveRoundTrip()`), matching `recipientQuery`'s own existing pattern exactly.

**Starting/Ending Done Date.** Replaces the single "Before Done Date" field outright with `startingDone`/`endingDone` — both optional, either end alone is a valid one-sided range (`r.doneISO >= startingDone` / `r.doneISO <= endingDone`). Both keep the existing calendar-picker-only behavior (`openPicker` + the Tab-only keydown guard) — this is still the one date-range in the app with that restriction, per the file's own header comment. `criteriaText` (the print report's Selection Criteria line) and the empty-state "Enter ... above to see records" message were both updated to name all four filters.

**Fixed instructional text.** The old `archnote` paragraph was action-dependent (three separate sentences keyed off `action`, plus a per-Delete "permanent, cannot be recovered" warning). Replaced with Jim's one fixed sentence covering all three Actions: "Select records to Archive, UnArchive, or Delete. Any combination of some or all of the search options below can be used to see a list of qualifying records. Archive selects active records marked as Done. UnArchive and Delete select Archived records." The Delete-specific permanence warning is dropped from here — the Delete confirmation modal (`handleDeleteSelected`'s own dialog, 2026-09-01) already states that warning independently, so nothing is lost.

**"From..." subtitle — flagged, not silently resolved.** Jim's own literal wording ("for Archive and Delete it would be 'From Archived Records'") is very likely a typo for "for UnArchive and Delete" — as written, it has Archive mode showing both "From Active Records" (his first sentence) and "From Archived Records" (his second) at once, which can't both be true. Cross-checked against the existing `rows` filter (`action === 'archive' ? r.done_date && !r.archived_at : !!r.archived_at`): Archive mode has always shown Done-but-not-yet-archived "active" records; UnArchive and Delete both show already-archived records — this is the data-driven reading, and the one implemented. Rendered as a `.subnote` line directly under each `listTitle` band: "From Active Records" when `action === 'archive'`, "From Archived Records" otherwise. Worth a quick confirmation from Jim rather than assumed final — flagged in `ArchiveForm.tsx`'s own header comment too.

No migration — every new field filters client-side over data already fetched. `npx tsc --noEmit`/`npm run lint` clean. No mockup updated — `design/screens/WYP_archive_palette1.html` still models the single-field version; flagged in `design/README.md`.

\---

## 2026-09-03 — Help-chip topic screens built (Getting Started / Creating a Request / Responding to a Request / ToDo Features)

Jim: "Could you write up an introduction for Private Testing participants which gives them a quick look at what is available with WYP — something focused on how to be more efficient by using the Would You Please app? Can you create screen or report sample images to include? As I think about it, instead of a document — one or several HTML pages under the Help chip would be ideal. Your thoughts?" Recommended building it as in-app Help pages reached from Main Screen's existing (but inert) Housekeeping Help-tab rows, rather than a downloadable document — those three rows had sat as "placeholder, no video linked yet" since the Tasks/Help chip toggle itself was built, and content that lives where the app already points to it doesn't go stale the way a separate PDF or doc would. Jim confirmed: "The three topics work, and add a ToDo features item" — keeping Getting Started / Creating a Request / Responding to a Request as-is and adding a fourth ToDo Features topic.

**Routing and shell.** Four new routes (`app/help/getting-started`, `.../creating-a-request`, `.../responding-to-a-request`, `.../todo-features`), each a thin `page.tsx` wrapping a component in `RequireAuth` (these are reached only from the signed-in Main Screen). New shared `HelpTopicShell.tsx` reuses the app's own `.frame-none`/`.app`/`.band`+`.glabel`/`.scroll` in-app screen shell — the same pattern `AccountForm.tsx`/`ArchiveForm.tsx` use — rather than `/privacy`'s own wide reading-width layout: Privacy is reached from a signed-out landing-page footer link and reads like an external legal document, while Help is reached from inside the authenticated mobile UI and should feel like part of the app. Close uses `router.back()`, same Detail-screen convention, returning to Main Screen with its Help tab still selected (`hkTab` persists regardless of which topic was open).

**Content and sample images.** Each topic is prose-first (per Jim's own CLAUDE.md-recorded formatting preference — headers/lists only where genuinely listy), built around one schematic SVG "sample screen" per topic, using the app's own `:root` design tokens (brand blue `#2A5FC8`, ink, row tint, alert red for overdue) rather than the literal hex values from any other source. These are hand-drawn schematics, not real screenshots — no headless browser is reachable from this session to capture the live, authenticated app, and a labeled schematic can call out just the feature the topic is discussing (e.g. the Reminders-until-Done checkboxes on the Create Request sample, or the quick-Done band on the Request Response sample) without the visual noise of a literal 1:1 capture. Content was checked against the app's actual current entitlements rather than assumed from memory — notably, Attachments and Repeat are both free-tier-available with caps (100 MB / 5 occurrences) per the 2026-08-27 free-tier expansion, not Subscriber-exclusive, so the Creating a Request and ToDo Features topics describe them that way rather than as paywalled features.

**New shared CSS** (`.help`/`.help-lead`/`.help-shot`/`.help-next`, §6.46 PROPOSED, `app/globals.css`) reuses existing global components where they already fit — `.panel` for the sample-image frame (the same white-card-plus-rule-border treatment Dialog/Attachments panels already use), plain `h2`/`p`/`ul` prose rules modeled on `/privacy`'s own typographic scale rather than a third, differently-sized prose system. A small shared `helpTopics.ts` names all four topics once, so the "what to try next" nav at the bottom of each topic (`HelpNext.tsx`, cycling to the next topic) can't drift out of sync with the route folders or `MainScreen.tsx`'s own row labels.

**Main Screen wiring.** The three existing placeholder `.hkrow` elements in the Help tab (inert `role="button"` divs with no `onClick`) now `router.push` to their real routes, using the identical onClick/onKeyDown-Enter pattern the Tasks tab's own rows (Contacts/Account Options/Archive/Install) already use; a fourth "ToDo Features" row was added the same way. The "— placeholder, no video linked yet" wording is gone.

`npx tsc --noEmit`/`npm run lint` clean. No migration needed — this batch reads no new data, only existing static content. No mockup — same reasoning as `/privacy`: this is prose-and-image content, not a Palette-1 form/button screen, flagged in `design/README.md` rather than silently skipped.

\---

## 2026-09-03 — Two earlier same-session fixes, logged here for the record: singular/plural change-notification wording, UTC-vs-local date bug on Dialog/created_at

Two small code fixes made earlier in this session, not yet captured in this log at the time.

**Singular/plural fix.** Jim: "The notification email should be singular/plural sensitive in the text 'The following data field for this Request has changed: Due Date.' or 'The following data fields for this Request have changed: Due Date, Dialog.'" The 2026-09-02 change-notification batch's wording was always plural regardless of count. New shared `changedFieldsSentence(fields, list)` helper in `app/src/lib/email.ts`, used by `emailChangedFieldsBox()` (the HTML box) and both `buildRequestEmailText()`/`buildOwnerUpdateEmailText()`'s plain-text equivalents, so the one field/fields distinction is written once rather than three times.

**UTC-vs-local date bug.** Jim: "I am testing a sent Request. Changes are not creating emails for Dialog or Description. Dialog entries are now being dated 9/3/26. Am I set up for a wrong time zone?" Root cause: `formatMDY(value)` — a helper written for date-only Postgres columns (`due_date`/`done_date`, no time-of-day or zone component) that reads the date via `value.slice(0, 10).split('-')` — was also being used on genuine `timestamptz` columns (`dialog.created_at`, and `requests.created_at` on Main Screen/Archive's own "Date" column), where slicing an ISO timestamp string reads its **UTC** calendar date rather than the viewer's **local** one. In the evening, for any negative-UTC-offset zone (all US zones), a timestamp already past midnight UTC but not yet midnight locally shows tomorrow's date — exactly what Jim saw with a Dialog entry dated one day ahead. Fixed by adding a second helper, `formatMDYFromTimestamp(value)` (built from `new Date(value).getFullYear()/getMonth()/getDate()` — local-time-based getters, not string slicing), and repointing every `created_at` display to it: `RequestDetailForm.tsx`, `RequestResponseForm.tsx`, `ResponseDetailForm.tsx`, `TodoDetailForm.tsx`, `MainScreen.tsx` (6 sites: Sent/Received/ToDos live rows and their print-report equivalents), `ArchiveForm.tsx` (3 sites, one per Record Type). `formatMDY` itself is untouched and still correct everywhere it's still used, for genuine date-only columns. Three files (`RequestDetailForm.tsx`/`RequestResponseForm.tsx`/`ResponseDetailForm.tsx`) had their now-fully-superseded original `formatMDY` deleted outright, since their only call site was the one that moved.

The original report ("changes are not creating emails for Dialog or Description") turned out to be a separate, non-code issue — confirmed via Vercel production log inspection (4 successful 200 responses, no runtime errors) and Jim's own follow-up that the emails had landed in his Gmail spam folder, on a still-reputation-building sending domain. No code change was needed for that part.

`npx tsc --noEmit`/`npm run lint` clean for both fixes.

\---

## 2026-09-02 — Change-notification "UPDATED:" emails, both directions, built end to end

Same-day follow-up to the `contentChanged` batch below. Jim, with a mocked-up email screenshot ("UPDATED:" subject prefix, a highlighted notice box reading "The following data fields for this Request have changed: Due Date, Dialog," directly above the existing CTA button): "I think all changes warrant an email to the other party. If it tells them what changes, they can decide whether or not to use the Request link. Since the standard email format shows the Request Description and Due Date, that may be sufficient visibility of the change. ... We don't currently need an option not to send an email." Confirmed first that no change-triggered email existed anywhere in this codebase — `send-request/route.ts` only ever fires once at creation, and the Reminder/Overdue templates are all schedule-triggered, not edit-triggered — so Request Detail's own long-standing `.noticeband` claim ("The Request Recipient is notified of changes.") had been aspirational, not real, until this batch.

**Two directions, both built.** Owner edits an existing Request (Request Detail) → notify the Recipient. Recipient edits (Response Detail, signed in, or Request Response, anonymous `/r/[token]`) → notify the owner/Requestor. No schema change was needed for either — everything reads off columns and RPCs that already exist.

**Templates (`app/src/lib/email.ts`).** `buildRequestEmailSubject` gained a fourth `'updated'` kind (`"UPDATED: "` prefix, same pattern as `'reminder'`/`'reminder_day_of'`). `RequestEmailBodyFields` gained an optional `changedFields?: string[] | null` — when present, `buildRequestEmailHtml`/`Text` render a new `emailChangedFieldsBox()` (a white callout, matching `emailDescriptionBox()`'s own visual treatment against the Strip-colored body) holding the "have changed: **X**, **Y**." sentence directly above the existing button, both inside one box — this is the recipient-facing direction, reusing the exact same template the Initial Request/Reminder emails already use, so it never drifts from them. The owner-facing direction is a new, separate template pair (`buildOwnerUpdateEmailSubject/Html/Text`) — deliberately not a reuse of the recipient-facing one, since "from `<ownerName>`... mark as completed" framing and a `/r/[token]` link are both wrong for an email arriving in the owner's own inbox about their own Request; this one names the Recipient instead ("to `<recipientName>`"), matching the existing Requestor digests' own convention, and links straight to `/requests/[id]` — no token needs minting for this direction, since only the owner can open that route (RLS owner-only).

**`CHANGED_FIELD_LABELS`** — one canonical, exported vocabulary (Due Date, Due Time, Description, Category, Done Date, Done Time, Dialog, Attachments) and a `sanitizeChangedFields()` allow-list validator, since the changed-field list is the one piece of information these routes trust from the client by necessity (the server has no way to reconstruct "what just changed" from the row's current state alone) — a forged or malformed field name is silently dropped before it can reach an email sent to a third party's inbox. **Deliberately excludes the three Reminders-until-Done checkboxes and Repeat** — neither is something Jim's own mockup or message described reporting, and both read as the sender's own internal preference rather than something meaningful to tell the other party about; flagged as a scoping call, not a literal instruction, easy to add later. The mockup's own trailing comma after "Dialog," was treated as an artifact of his draft, not reproduced — the real sentence ends in a period.

**Two new API routes.** `app/api/email/send-request-update/route.ts` (owner → Recipient) mirrors `send-request/route.ts`'s own forwarded-JWT posture exactly — no service_role needed, since the caller already owns the Request under RLS. `app/api/email/send-request-update-to-owner/route.ts` (Recipient → owner) genuinely needs service_role, the same justified exception CLAUDE.md already carves out for the Attachments routes and the cron route: the caller here is the Recipient, and the email has to reach the owner's own account email, which only `auth.admin.getUserById` can read. Verifies the caller's permission *first*, using only an anon-key client (plus the caller's own forwarded JWT for the signed-in case) — a `token` is verified via the existing anon-callable `get_request_by_token` RPC, a `requestId` + forwarded JWT via the existing `get_received_request` RPC — before ever touching service_role; a caller who couldn't open Response Detail or `/r/[token]` for a given Request can't get an email sent through this route either.

**Field-diff tracking, wired into all three screens.** `RequestDetailForm.tsx`, `ResponseDetailForm.tsx`, and `RequestResponseForm.tsx` each gained a `computeChangedFieldLabels()` — diffing the live form against the same load-time snapshot (`initialFormRef`) each screen's own `hasChanges`/Close-Cancel-label logic already maintains (or, for Request Response, a new snapshot built purely for this purpose — that screen never had one, since it has no Cancel button to gate) — plus two new flags, `dialogChanged`/`attachmentsChanged`, split out of the same-day `contentChanged` flag at the identical two call sites (Dialog-modal save, `AttachmentsPanel`'s `onContentChange`). `sendChangeNotification()` on each screen is fire-and-forget, called only after the underlying Save/Send RPC already succeeded — same "never block or undo an already-saved edit" posture `CreateRequestForm.tsx`'s own Initial Request email call already takes. `RequestDetailForm.tsx`'s field diff is gated on `categoriesEnabled`/`requestTimeEnabled` so a hidden field's stored value never appears in the list purely because it happens to differ from a prior visit.

`npx tsc --noEmit`/`npm run lint` clean for the whole batch. No mockup changes — none of the three screens' static HTML has interactive Save/Send JS to notify from. No migration needed.

\---

## 2026-09-02 — Send/Save button gated on Dialog/Attachments too (`contentChanged`); migration 050 fixes Response Detail's Answer error

Jim, with three screenshots: (1) "In Request Detail and in Response Detail, the Send button continues to be disabled after a Dialog or Attachment is added - unless a text or date is changed." (2) A real SQL error opening Response Detail and answering a Question there ("column 'subject_id' is of type uuid but expression is of type bigint"), which he could not reproduce on Request Detail.

**Bug 1 root cause**: the same-day `hasChanges` dirty-check batch (#611, gating the Send/Save button on whether anything changed) reused the existing `hasChanges` `useRef` snapshot verbatim. That snapshot was deliberately built 2026-08-20 to exclude Dialog/Attachments, since those already save independently of Send/Save and shouldn't affect the Close-vs-Cancel *label*. Reusing the same flag to also gate the *button* was never the snapshot's intended purpose, and produced exactly what Jim saw. Fix: a second, separate `contentChanged` boolean per screen — set by a successful Dialog-modal save, and by a new `onContentChange` callback prop on the shared `AttachmentsPanel` component (fired on upload success, Location/reference-add success, and delete success — deliberately *not* fired by the Repeat-carry checkbox toggle, which isn't new content). The Send/Save button's `disabled` prop is now `saving/sending || (!hasChanges && !contentChanged)`; the Close/Cancel *label* logic is untouched, still reading `hasChanges` alone. Applied to `RequestDetailForm.tsx`, `ResponseDetailForm.tsx`, and `TodoDetailForm.tsx` (all three Detail/edit screens with a dirty-check-gated Send/Save button); `AttachmentsPanel.tsx` gained the new optional `onContentChange` prop.

**Bug 2 root cause, migration 050 (confirmed run by the owner, 2026-09-02)**: byte-for-byte the same uuid/bigint `events.subject_id` bug migration 010 already found and fixed once, in `add_dialog_by_token` (2026-08-10) — but `add_dialog_as_recipient` was written a day later (migration 012, 2026-08-11) and the fix was never ported across to it. Jim's own report explains why Request Detail worked fine for him: the owner's own screens write Dialog with a plain client insert, never through this RPC — only a signed-in recipient answering via Response Detail (`get_received_request`'s own write path) ever calls `add_dialog_as_recipient`. Fix, identical in shape to migration 010's: log the `events` row with the Request's own uuid as `subject_id`, not the new dialog row's bigint id (preserved instead in `detail->>'dialog_id'`). The Answer error is now fixed in production.

`npx tsc --noEmit`/`npm run lint` clean for the `contentChanged` batch. No mockup changes — none of the three screens' static HTML has interactive Send/Save-disabling or Dialog/Attachments JS to update.
...
## 2026-09-07 — Admin Statistics: full build (migrations 053-060)

Jim: "I would like to consider building an admin app for WYP for which initially only I have access to," refined over a long design conversation into `docs/WYP_Admin_Statistics_Plan.md`, then "let's start." Built end to end in one pass, in the plan's own build order.

**Access.** `profiles.is_admin boolean not null default false` (migration 053) — a single column, set only by hand via SQL, reusing migration 051's own precedent of a boolean flag over a third allowlist table. Housekeeping gains a third chip, **Statistics**, rendered only when the signed-in profile's `is_admin` is true, listing four rows (Contacts, Requests, ToDos, Sum and Averages) that route to `/admin/stats/*`, each wrapped in a new `RequireAdmin.tsx` guard. The chip is UX only — every admin RPC below re-checks `is_admin` itself.

**Events instrumentation.** `public.events` (migration 002) already existed but only covered the recipient/secure-link side. Migration 054 adds `log_event()`, a narrow SECURITY DEFINER RPC letting the browser log an event for a Contact/Request/ToDo it actually owns, and instruments the sites that needed it: Contact created (`AddContactForm.tsx`, with `has_phone`/`has_notes` captured at creation time) and deleted (`delete-cascade` route); Request/ToDo deleted (`delete-many` route); Request "Changed"/"Done" in both directions (`send-request-update`/`send-request-update-to-owner` routes — logged once the route confirms a real edit happened, not gated on the email actually sending); ToDo created/Changed/Done (`CreateTodoForm.tsx`, `TodoDetailForm.tsx`). Two gaps surfaced later, while building the screens that needed them, and were fixed the same way: Request creation (`CreateRequestForm.tsx`) had no events write at all, and Attachment upload likewise — both now log a `created` event via the same pattern. Migration 055 adds `set_archived()`, replacing `ArchiveForm.tsx`'s plain client-side `.update()` with a SECURITY DEFINER function that logs `archived`/`unarchived` alongside the write — the one genuinely new server code path this build needed, per the plan's own flag.

Hard deletes (Contacts, Requests/ToDos, individual Attachments) mean anything reconstructed from *current* table state silently undercounts what's since been removed — Added/Deleted/Changed/Done/Archived/Unarchived/Attachments-added all read from events specifically to avoid that. Dialog has no delete path anywhere in the app, so it's read straight from `public.dialog` with no events involved. Weeks/months before an event type started being logged show blank for that metric, per the plan's "no retroactive estimation" decision — confirmed, not guessed.

**The four screens.** Contacts, Requests, and ToDos Activity (migrations 056/058/059) share `AdminStatsShared.tsx`: a filter bar (From/To month, Weekly Sun–Sat/Monthly, cohort — All/Beta allowlist/one account via the new `admin_list_profiles()`, migration 057), and hand-rolled inline-SVG chart primitives (bar, diverging, line) baked into one shared scrolling `<svg>` per screen so every metric shares one x-axis and scrolls in sync, plus a plain data table that doubles as the print and export source. Requests adds a Sent/Received toggle (Created, Changed, Done, Attachments, Dialog split both ways; Deleted/Archived/Unarchived stay Sent-only) — "Received" matched by the same email-to-account join `get_received_requests()` uses internally, generalized across a whole cohort rather than one caller's own session. Sum and Averages (migration 060) is a point-in-time snapshot instead — an "as of" date rather than a range, two RPCs (`admin_stats_summary_totals`/`_roster`) rendering as KPI stat tiles and a per-account roster table rather than charts. Its roster reconstructs Open/Done/Archived from `done_date`/`archived_at` for any past date, but Overdue only when "as of" is today — the same permanent limitation the plan already gives for the Activity screens' own Overdue, now documented at this screen's own migration too.

Every screen has a Print button (table-only print output, not the on-screen chart — its `overflow-x:auto` scroller would otherwise clip to whatever's currently scrolled into view) and an **Export .xlsx** button hitting a new `/api/admin/stats/export` route (added the `exceljs` dependency), one workbook per screen mirroring exactly whatever's on screen, per the plan's decision against a separate "full range" option.

**Owner action required**: migrations 053–060 need to be run in the Supabase SQL editor (`docs/Week6 - SQL history.txt`), and at least one profile needs `is_admin = true` set by hand — confirmed done for `jimgillon@gmail.com`. Deployed via a normal commit + `git push origin main` (this session's first attempt hit two unrelated snags along the way: a stale `.git/index.lock` blocking `git add`, and the fact that the code initially existed only as uncommitted local changes, so the very first "trigger deploy" push carried nothing new — both resolved, and the Statistics chip is now confirmed live).

`npx tsc --noEmit`/`npm run lint` clean throughout, at every migration. `npm run build` could not be verified end-to-end in this session's own sandboxed shell (no network route to Google Fonts there, unrelated to any of this code) — Vercel's own build already succeeded per Jim's confirmation that the chip is live.

---

## 2026-09-07 — Admin Statistics bugfixes: two ambiguous-column errors (migrations 061-062)

Live testing surfaced two runtime-only Postgres errors in the Admin Statistics build (migrations 053-060) that `tsc`/lint and function creation itself couldn't catch, since PL/pgSQL only binds embedded SQL lazily, on first execution — not at `create function` time. Both are the same root cause: a function's own `returns table(...)` column name creates an implicit PL/pgSQL OUT variable with that name, visible everywhere in the function body, and an *unqualified* reference to a same-named table column elsewhere in the function collides with it. Postgres reports this as "column reference is ambiguous," not as a missing-column or type error, which is what made it non-obvious from the code alone.

**Migration 061** — `admin_stats_summary_roster()` (Sum and Averages roster) declares an `email` output column; its cohort CTE's beta-allowlist check, `select lower(email) from public.beta_allowlist`, left `email` unqualified. Fixed by aliasing: `public.beta_allowlist ba` / `lower(ba.email)`. The identical snippet appears verbatim in migrations 056/058/059/060's totals function too, but none of those declare an `email` output column, so only the roster function was actually broken.
...
## 2026-09-07 — Sum and Averages reformat (migration 065)

Jim reviewed the Sum and Averages screen and sketched a reformat: a single scrolling Entities table (Accounts, Contacts, Req's Sent, Req's Received, ToDos — Count Total, Avg Descr char's, and Open/Overdue/Done/Archived as whole percentages, rounded) replacing the old Grand-Totals stat-tile block; a new Volume table (Dialogs, Attachments — Count Total, Avg Descr/Size, and Average count per Account/Sent Req/Rec'd Req/ToDo); and a simplified per-account Roster (Account, Contacts, Req Sent, Req Rec'd, ToDos, Dialogs, Atch's, Avg Atch Size), dropping the roster's old per-account Open/Overdue/Done/Archived breakdown — that detail stays reachable by switching the existing Accounts filter to a specific account. Worked through as a full design proposal (recommendation, rejected alternatives, open questions) before building, landing on: Account shown as email (not display name); Attachments' Avg Size in KB, rounded whole; the (1)/(2) calculation markers on Jim's sketch are internal documentation only, not on-screen footnotes. Captured as a new standalone document, `docs/WYP_Admin_Statistics_Specification_v1_0.docx` (also written to the project's knowledge base as a Markdown mirror, `claude/WYP_Admin_Statistics_Specification_v1_0.md`, since an automated .docx upload to the project failed) — the first formal spec for the Admin Statistics screen family, since none of these five screens had ever been given one; the other four (Accounts/Contacts/Requests/ToDos Activity) are listed in it as build references pending their own future sections.

**Real gap closed, not just a reformat.** `admin_stats_summary_totals()`/`admin_stats_summary_roster()` (migration 060) never tracked ToDos Overdue anywhere — the totals RPC had no per-status breakdown at all, and the roster's own `todos_*` columns had `open`/`done`/`archived` but no `overdue`, even though the main app's ToDos filter chips have supported a real Overdue state since 2026-08-12 ("ToDos gain an Overdue chip"). Migration 065 adds `todos_overdue` to the totals RPC using the identical "not archived, not done, due_date < current_date, only ever nonzero when `p_asof` = today" rule already used for Requests, so ToDos get the same four-way status breakdown Requests do. (The PRD §3.4/UI spec §6.2 text that still says "Overdue does not apply to ToDos" is a separate, already-flagged documentation correction from 2026-08-12 — not touched by this migration.)

**`admin_stats_summary_totals()` rewritten** to return raw counts rather than pre-computed figures: `accounts_count`, `contacts_count`, then for Req's Sent/Req's Received/ToDos each: `*_total`, `*_open`, `*_overdue`, `*_done`, `*_archived`, `*_avg_description`; plus `dialog_total`/`dialog_avg_description` (average character length of Dialog `body`, across Requests+ToDos owned by the cohort — same scope migration 060 originally used, not expanded to received-side Dialog) and `attachments_total`/`attachments_avg_size_kb`. Percentages and the Volume table's four "average count per" ratios are computed client-side (`AdminSummaryStatsForm.tsx`'s `pct()`/`ratio()`) and again in the export route with matching logic, rather than baked into the RPC — one rounding rule shared by the screen, its print view, and the .xlsx export, instead of three copies. `ratio()` rounds to one decimal place (the pre-existing `avg_dialog_per_request` precedent); the spec didn't give this a rounding rule, so flagged to Jim as an assumption rather than picked silently.

**`admin_stats_summary_roster()` simplified**: drops `display_name` (Account is email-only per Jim's decision) and every per-account Open/Overdue/Done/Archived column, down to `account_id`, `email`, `contact_count`, `requests_sent_total`, `requests_received_total`, `todos_total`, `dialog_count`, `attachments_count`, `attachments_avg_size_kb`.

**Both RPCs re-checked against the migration 061/062 ambiguous-column pitfall** (a `returns table(...)` column name shadows a same-named unqualified column reference elsewhere in the function body): the roster still declares an `email` output column, so its cohort CTE's beta-allowlist check stays qualified (`ba.email`), matching the migration 061 fix; neither function declares an `id` output column, so the standing `where id = auth.uid()` admin-gate line is harmless in both, per migration 062's own note.

**`StatTile` removed** from `AdminStatsShared.tsx` (and its `.stattilerow`/`.stattile`/`.stattilevalue`/`.stattilelabel`/`.stattilesub` CSS from `globals.css`) — it existed only for Sum and Averages' old Grand-Totals block, which no longer exists, and nothing else in the app used it. New `.stattable-locked` CSS instead: the row-label/Account column stays put via `position: sticky; left: 0` inside each table's existing `overflow-x: auto` wrapper, while the rest of the row scrolls — the mechanism the spec's §2.2 calls for, added as a small extension of the existing `.stattablewrap`/`.stattable` pattern rather than a new component.

**Also fixed, same session, unrelated to the reformat itself**: Main Screen's Housekeeping row under Statistics read "Sum and Averages — grand totals and per-user roster" — a leftover from before the roster was per-account; corrected to "per-account roster" (`MainScreen.tsx`).

**Owner action required**: migration 065 is written to `docs/Week6 - SQL history.txt` per the standing convention but not yet run — this session's device shell was unavailable ("Workspace unavailable... isolated Linux environment... failed to start"), so `npx tsc --noEmit`/`npm run lint` could not be run here either. Run the migration in the Supabase SQL editor, then verify `tsc`/lint clean before deploying — this is the first change to this screen family that has not been machine-verified in the same session it was written.
