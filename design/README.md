...
| Request Detail | `screens/WYP_request_detail_palette1.html` | `/requests/[id]` | **Live** (`app/components/RequestDetailForm.tsx`, 2026-08-09). Issuer's own view of a Request already sent — built from Create Request's mockup. Recipient shown as non-modifiable label:value text (`.metarow`, not a boxed field — §6.28); Due Date/Time, Done Date/Time, Category, Description all fetch pre-filled and update on Send. Notice band (Strip background) under the title: "Note: The Request Recipient is notified of changes." Dialog panel reads the real `dialog` thread (not a staged list) and each Add Dialog Save writes immediately, since the Request already exists; Add Dialog's Kind picker is dynamic (open-Question rule) — Answer unlocks only when a Question in the thread is still open, with a which-Question picker above one open Question. Selects `dialog.replies_to_id` (migration 006, confirmed run by the owner 2026-08-09). Action buttons stay Send/Cancel, unchanged. Both return via `router.back()`, not `push('/')` — see the Main Screen entry above |
